...
async function getDatasAndFilterIt(artistName) {
  ...
  try{
    ...
    const filteredData = newDatas.filter(song => song.artists.find(artist => artist.fullName === artistName));
    ...
    if(filteredData.length > 0){
      ...
      container.innerHTML = '<p class="content__not-found">No Musics Found !</p>';
    }
    ...
    musicCardTemplate = `
         <div class="music-card">
                ...
                <div class="music-card__informations">
                  <a href="/pages/singlemusicpage.html?id=${song.id}" class="informations__music-name">${song.title}</a>  
