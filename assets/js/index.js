...
import { myFetch } from "./utils/interceptFetch.js";
...
async function getAllSongs() {
	...
	try {
		const datasObj = {
			newMusics: (await myFetch("https://haji-api.ir/music/?q=new")).results,
			topArtists: (await myFetch("https://haji-api.ir/music/?q=trend")).results,
			dailyMusics: (await myFetch("https://haji-api.ir/music/?q=day")).results,
			weeklyMusics: (await myFetch("https://haji-api.ir/music/?q=week")).results
		};
...
<img loading="lazy" class="artist-card__img" src="${currentData.image.cover
						.url}"/>
</div>
...
<img loading="lazy" class="music-card__img" src="${currentData.image.cover
						.url}" />
